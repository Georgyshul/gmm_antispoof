...
def extract_mfcc(file, num_ceps=20, order_deltas=2, low_freq=0, high_freq=4000):
    sig, fs = sf.read(file)
    # put VAD here, if wanted
    mfccs = mfcc(sig=sig,
                 fs=fs,
...
def extract_features(filename, features, cached=False):
    def get_feats():
        if features == 'lfcc':
            return extract_lfcc(filename)
        elif features == 'mfcc':
            return extract_mfcc(filename)
        else:
            return None
    ...
    if cached:
        ...
        group = h5.get(filename)
        if group is None:
            data = get_feats()
            h5.create_dataset(filename, data=data, compression='gzip')
        else:
            data = group[()]
        h5.close()
        return data
...
def train_gmm(data_label, features, train_keys, train_folders, audio_ext, model_file, ncomp, init_only=False):
    logging.info('Start GMM training.')

    partial_gmm_model_file = '_'.join((model_file, data_label, 'init', 'partial.pkl'))
    if exists(partial_gmm_model_file):
        gmm = GaussianMixture(covariance_type='diag')
        with open(partial_gmm_model_file, "rb") as tf:
            gmm._set_parameters(pickle.load(tf))
    else:
        data = list()
        for k, train_key in enumerate(train_keys):
            files_keys = pd.read_csv(train_key, sep=' ', header=None)
            files = files_keys[files_keys[4] == data_label][1]
        ...
        with open(partial_gmm_model_file, "wb") as f:
    ...
    for i in range(10):
        partial_gmm_dict_file = '_'.join((model_file, data_label, str(i), 'partial.pkl'))
...
def scoring(scores_file, dict_file, features, eval_ndx, eval_folder, audio_ext, features_cached=True, flag_debug=False):
    ...
    list_of_files = pd.read_csv(eval_ndx, sep=' ', header=None)
    if flag_debug:
        list_of_files = list_of_files[:1000]

    files = list_of_files[1].values
...
#     pd = pd.read_csv(eval_ndx, sep=' ', header=None)
#     if flag_debug:
#         pd = pd[:1000]
...
#     pd_out = pd.DataFrame(scr).set_index(files)
